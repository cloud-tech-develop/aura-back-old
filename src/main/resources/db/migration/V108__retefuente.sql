...
-- ═══════════════════════════════════════════════════════════════════════════
-- NOTAS PARA EL MOTOR — la depuración de la base es lo difícil
--
-- ORDEN LEGAL (no se puede alterar):
--
--   1. Ingreso bruto del mes (todo lo devengado)
--   2. − Ingresos NO constitutivos de renta:
--         · aporte obligatorio a salud
--         · aporte obligatorio a pensión
--         · aporte a fondo de solidaridad
--   3. − Deducciones (empleado_deduccion_renta), con sus topes:
--         · dependientes: 10% del ingreso bruto, máx 32 UVT/mes
--         · intereses de vivienda: máx 100 UVT/mes
--         · medicina prepagada: máx 16 UVT/mes
--   4. − Rentas exentas (AFC, AFP voluntario)
--   5. = Subtotal
--   6. − 25% EXENTO del subtotal, con TOPE DE 790 UVT ANUALES (≈65.83 UVT/mes)
--   7. ⚠️ TOPE GLOBAL: (deducciones + rentas exentas) no puede exceder el 40%
--         del resultado del paso 2, ni 1.340 UVT anuales.
--   8. = Base gravable → convertir a UVT → buscar rango → aplicar fórmula
--
--   retencion_uvt = (base_uvt - uvt_resta) * tarifa/100 + uvt_suma
--   retencion_pesos = retencion_uvt * uvt_valor  → redondear a mil más cercano
--
-- PROCEDIMIENTO 1: se calcula así cada mes, sobre el ingreso del mes.
--
-- PROCEDIMIENTO 2: en junio y diciembre se promedian los 12 meses anteriores
--   para obtener un porcentaje fijo (retefuente_porcentaje_fijo), que se
--   aplica sobre el ingreso gravable de cada mes del semestre siguiente.
--   Si el empleado tiene menos de 12 meses, se promedia lo que lleve.
--
-- TESTS OBLIGATORIOS (validados con contador):
--   · borde de cada rango UVT
--   · el tope de 790 UVT del 25% exento
--   · el tope global del 40%
--   · un empleado que no alcanza a retener (base < 95 UVT → 0)
--   · procedimiento 2 con menos de 12 meses de historia
--
-- Retefuente mal calculada es un problema con la DIAN, no un bug cosmético.
-- ═══════════════════════════════════════════════════════════════════════════
