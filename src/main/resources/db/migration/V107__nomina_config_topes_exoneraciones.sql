...
-- ═══════════════════════════════════════════════════════════════════════════
-- NOTAS PARA EL MOTOR (orden de aplicación)
--
--  1. baseIbc = salarioProporcional + novedades con constituye_ibc
--               (SIN auxilio de transporte — bug B1)
--
--  2. Si contrato.es_salario_integral:
--        baseIbc = baseIbc * factor_salario_integral / 100      (70%)
--
--  3. baseIbc = MIN(baseIbc, tope_ibc_smmlv * smmlv)            (tope 25 SMMLV)
--
--  4. Deducciones empleado: salud 4%, pensión 4% sobre baseIbc
--
--  5. Si aplica_fondo_solidaridad y baseIbc >= 4 SMMLV:
--        buscar rango en fondo_solidaridad_rango
--        descontar (pct_solidaridad + pct_subsistencia) sobre baseIbc
--
--  6. Aportes empleador sobre baseIbc:
--        - pensión 12%  → SIEMPRE
--        - ARL          → SIEMPRE (tarifa por nivel de riesgo)
--        - CCF 4%       → SIEMPRE
--        - salud 8.5%   ┐
--        - SENA 2%      ├→ CERO si aplica_exoneracion_1607
--        - ICBF 3%      ┘   AND devengado < umbral_exoneracion_smmlv * smmlv
--
--  ⚠️ La exoneración se evalúa POR EMPLEADO (su devengado), no por empresa.
--     Un mismo cliente puede tener empleados exonerados y no exonerados.
--
--  ⚠️ La exoneración NO aplica a CCF ni a pensión. Solo salud empleador,
--     SENA e ICBF.
--
--  TESTS OBLIGATORIOS: el borde de cada rango, el tope exacto de 25 SMMLV,
--  y el umbral exacto de 10 SMMLV (¿< o <=?  → verificar con contador).
-- ═══════════════════════════════════════════════════════════════════════════
