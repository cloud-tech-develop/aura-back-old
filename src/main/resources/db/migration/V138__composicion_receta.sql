...
--
-- `cantidad` conserva exactamente su semántica anterior, así que VentaServiceImpl
...
-- FKs sin ON DELETE CASCADE a propósito: borrar una unidad de medida o una
-- presentación no debe borrar silenciosamente una línea de receta.
DO $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_composicion_padre ON producto_composicion(producto_padre_id);
-- El explosionado inverso ("¿en qué recetas entra esta harina?") y la
-- validación de ciclos recorren por hijo.
CREATE INDEX IF NOT EXISTS idx_composicion_hijo  ON producto_composicion(producto_hijo_id);

-- Un mismo hijo no puede estar dos veces en la misma receta: el guardado por
-- lote hace diff contra esta llave.
--
-- El servicio ya validaba la unicidad, pero la BD no: si alguna fila duplicada
-- se coló por otra vía, el CREATE UNIQUE INDEX tumbaría el arranque de la app.
-- Se consolidan sumando (dos filas del mismo ingrediente = el total que entra)
-- y se conserva la de menor id.
WITH duplicadas AS (
