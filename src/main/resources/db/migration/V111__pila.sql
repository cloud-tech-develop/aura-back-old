...
-- ═══════════════════════════════════════════════════════════════════════════
-- NOTAS — leer antes de estimar
--
-- 1. LOS IBC SON DISTINTOS ENTRE SUBSISTEMAS. No es una base por cuatro
--    tarifas. Un empleado en licencia no remunerada cotiza salud pero no
--    pensión; uno con incapacidad tiene IBC de ARL distinto al de EPS.
--
-- 2. LOS DÍAS TAMBIÉN SON DISTINTOS. dias_salud ≠ dias_pension ≠ dias_arl
--    según las novedades del mes. Hoy el motor tiene un solo
--    `dias_trabajados INT DEFAULT 30`.
--
-- 3. LAS BANDERAS SON DERIVADAS, NO CAPTURADAS:
--      ing      ← contrato.fecha_inicio dentro del período
--      ret      ← contrato.fecha_fin dentro del período
--      vsp      ← cambio en contrato_salario_historial dentro del período
--                 (POR ESO la Fase 2 es prerrequisito)
--      tde/tae  ← cambio de EPS en contrato_afiliacion dentro del período
--      tdp/tap  ← cambio de AFP
--                 (POR ESO la Fase 5.5 es prerrequisito)
--      sln/ige/lma/vac_lr ← novedades con sus pares de fechas
--
-- 4. ESFUERZO REAL: en el ERP de referencia, el servicio de PILA tiene ~3.900
--    líneas (solo consultarNovedades() ocupa ~500), y eso REUTILIZANDO motor
--    de conceptos, vinculaciones e histórico salarial que ya existían.
--    No es una fase de dos semanas.
--
-- 5. REDUCIR ALCANCE — investigar antes de escribir código:
--    ¿el operador (Aportes en Línea, SOI, ...) tiene API? Generar el plano de
--    ~120 columnas y lidiar con sus validaciones es donde se va el tiempo.
--    Pendiente #6.
-- ═══════════════════════════════════════════════════════════════════════════
