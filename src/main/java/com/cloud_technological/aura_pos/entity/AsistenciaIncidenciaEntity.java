package com.cloud_technological.aura_pos.entity;

import java.time.LocalDate;
import java.time.LocalDateTime;

import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.FetchType;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.JoinColumn;
import jakarta.persistence.ManyToOne;
import jakarta.persistence.Table;
import lombok.Getter;
import lombok.Setter;

@Getter
@Setter
@Entity
@Table(name = "asistencia_incidencia")
public class AsistenciaIncidenciaEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "empresa_id", nullable = false)
    private EmpresaEntity empresa;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "asistencia_dia_id")
    private AsistenciaDiaEntity asistenciaDia;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "empleado_id", nullable = false)
    private EmpleadoEntity empleado;

    @Column(name = "fecha", nullable = false)
    private LocalDate fecha;

    @Column(name = "tipo_incidencia", length = 40, nullable = false)
    private String tipoIncidencia;

    @Column(name = "descripcion", length = 255)
    private String descripcion;

    @Column(name = "estado", length = 30, nullable = false)
    private String estado = "PENDIENTE_REVISION";

    @Column(name = "requiere_soporte", nullable = false)
    private Boolean requiereSoporte = false;

    @Column(name = "soporte_url", length = 255)
    private String soporteUrl;

    @Column(name = "registrado_por")
    private Integer registradoPor;

    @Column(name = "fecha_registro")
    private LocalDateTime fechaRegistro;

    @Column(name = "revisado_por")
    private Integer revisadoPor;

    @Column(name = "fecha_revision")
    private LocalDateTime fechaRevision;

    @Column(name = "observacion_revision", length = 255)
    private String observacionRevision;
}
