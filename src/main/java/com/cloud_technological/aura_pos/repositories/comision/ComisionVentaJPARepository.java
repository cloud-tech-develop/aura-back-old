...
public interface ComisionVentaJPARepository extends JpaRepository<ComisionVentaEntity, Long> {
    ...
    // Pendientes de un técnico (liquidacion_id IS NULL)
    @Query("SELECT cv FROM ComisionVentaEntity cv WHERE cv.tecnico.id = :tecnicoId AND cv.empresa.id = :empresaId AND cv.liquidacion IS NULL")
    List<ComisionVentaEntity> findPendientesByTecnicoIdAndEmpresaId(
